...
//Conexión con firebase
        ...
        dbFirestore.collection("ToDo")
            .whereEqualTo("code", "true")
            .get().addOnSuccessListener {
                for (todo in it) {
                    var id = todo.id
                    datos.add(
                        Task(
                            id.toInt(), todo.get("code") as String,
                            todo.get("size") as String,
                            todo.get("color") as String,
                            todo.get("type") as String,
                            todo.get("price") as String
                        )
                    )
                }
        ...
        runBlocking {
            launch {
              ...
              for (todo in result){
                    datos.add(Task(todo.id, todo.code, todo.size,  todo.color, todo.type, todo.price))
                }
            }
        }
        var taskAdapter = TaskAdapter(datos){
